...
> **`NOTE:`** If the pre-installation is resuming here, ensure the environment variables have been properly set by following [Set reusable environment variables](pre-installation-cpi.md#13-set-reusable-environment-variables) and then coming back to this page.
...
1. (`pit#`) Merge the system-specific settings generated by CSI into `customizations.yaml`.
...
1. (`pit#`) Review the configuration to generate these sealed secrets in `customizations.yaml` in the `site-init` directory:
    ...
    >
    > See the `Decrypt Sealed Secrets for Review` section of [Manage Sealed Secrets](../../operations/security_and_authentication/Manage_Sealed_Secrets.md#decrypt-sealed-secrets-for-review), if needing to examine credentials from prior installations.
...
1. (`pit#`) Review the changes that you made.
    ...
    ```
        170c170
        <                 value: '{"Cray": {"Username": "root", "Password": "initial0"}}'
        ---
        >                 value: '{"Cray": {"Username": "root", "Password": "XXXX"}}'
        174c174
        <                 value: '{"SNMPUsername": "testuser", "SNMPAuthPassword": "testpass1", "SNMPPrivPassword": "testpass2"}'
        ---
        >                 value: '{"SNMPUsername": "testuser", "SNMPAuthPassword": "XXXX", "SNMPPrivPassword": "XXXX"}'
        182c182
        <                 value: '{"Username": "root", "Password": "initial0"}'
        ---
        >                 value: '{"Username": "root", "Password": "XXXX"}'
        190c190
        <                 value: '{"Username": "admn", "Password": "admn"}'
        ---
...
1. To customize the PKI Certificate Authority (CA) used by the platform, see [Certificate Authority](../../background/certificate_authority.md).

    > **`IMPORTANT`** The CA may not be modified after install.

1. (`pit#`) Set environment variables for the LDAP server and its port.

   The LDAP server hostname to be selected sepends on the IP address of master node. If last octet of IP address is even -
   
   ```bash
   LDAP=dcldap2.us.cray.com
   PORT=636
   ```
   If last octet of IP address is odd - 
...
1. (`pit#`) Configure the Unbound DNS resolver (if needed).
    ...
    If there is **no requirement to resolve external hostnames or no upstream DNS server**, then remove the DNS forwarding configuration from the `cray-dns-unbound` service.
...
1. `site-init` is now prepared. Resume [Initialize the LiveCD](pre-installation-cpi.md#36-initialize-the-livecd).
