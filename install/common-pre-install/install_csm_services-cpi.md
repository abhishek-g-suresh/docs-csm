...
> **`NOTE`**: In HPCM  error is expected, so the podman run commands mentioned in  `./steps/1.initialize_bootstrap_registry.yaml`  can be modified to run using `--privileged flag`.
   ...
   > **`NOTE`**
   > At the following step you may be prompted for a password:
   ```bash
     SUCCESS  Step: Set Management NCNs to use Unbound --- Checking Precondition
     Password: initial0
     Password: initial0
     Password: initial0
     root@ncn-m001's password: cmdefault
   ```
...
1. (`pit#`) Wait for the `spire-update-bss` job to complete.
   ...
   Expected Output  - 
   ```
   job.batch/spire-update-bss-1 condition met
   ```
...
1. After having waited until services are healthy (run `kubectl get po -A | grep -v 'Completed\|Running'` to see which pods may still be `Pending`), take a manual backup of all Etcd clusters. Following is expected - 
    ```bash
     # kubectl get po -A | grep -v 'Completed\|Running'
     NAMESPACE           NAME                                                              READY   STATUS      RESTARTS   AGE
     services            cray-crus-798cc799cf-97nwb                                        0/4     Init:0/2    0          15h
     services            cray-dns-unbound-manager-27861440-bhl4w                           1/2     NotReady    0          40s
    ```
...
1. (`pit#`) Check for running `sonar-sync` jobs. If there are no `sonar-sync` jobs, then wait for one to complete. The `sonar-sync` `CronJob` is responsible for copying the `sls-s3-credentials` secret from the `default` namespace to the `services` namespace.
